...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		//Skip Destino After Balance Verification
		GlobalVariable.BenefUnicoDetails = "True"
		
		if(Mobile.verifyElementVisible(findTestObject('EnvioTuEfectivo/Telephone object - Retiros'), 2, FailureHandling.OPTIONAL)) {
			String Telephone = Mobile.getText(findTestObject('EnvioTuEfectivo/Telephone object - Retiros'), 3, FailureHandling.OPTIONAL)
		...
		if(Description.contains('Amount less than 100')||Concept.contains('Amount less than 100')) {
		...
		//Datos screenshot
		if(Mobile.verifyElementVisible(findTestObject('ImpuestosYServicios/Monto - Title'), 2, FailureHandling.OPTIONAL)) {
			CustomKeywords.'helper.customfunction.Screenshot'()
		}
...
def PaymentVerification() {
	if (Verification == 'SelfAuthorize') {
		...
		GlobalVariable.destino_before_app = "No Info For Acct " + CuentaDestino
