...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	Mobile.delay(1)
	Mobile.tap(findTestObject('ImpuestosYServicios/Impuestos - Title'), 5, FailureHandling.OPTIONAL)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		if (Impuestos == '') {
			...
			println('Impuestos Not Available...!!!')
		} else {
			WebUI.callTestCase(findTestCase('CommonTestCases/TC_Servicios_SelectImpuestos'), [('Impuestos') : Impuestos, ('ReferenceNo') : ReferenceNo], FailureHandling.CONTINUE_ON_FAILURE)
		}
		...
		if(Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Digita un monto'), 5, FailureHandling.OPTIONAL)) {
			...
			try {
				...
				if(checkMonto.matches(".*[a-zA-Z]+.*")) {
					String preDefinedMonto = Mobile.getText(findTestObject('ImpuestosYServicios/Monto on Datos screen'), 1, FailureHandling.OPTIONAL)
					String Balance1 = preDefinedMonto.replaceAll('[,]', '')
					String Balance2 = Balance1.substring(4)
					GlobalVariable.MontoDecimal = Balance2
				}
			} catch (Exception e) {
				GlobalVariable.MontoDecimal = '200'
			}
			...
			'CHECK FOR IMPUESTOS/COMMISSION : '
			...
			KeywordUtil.logInfo('Monto Not Found..')
		}
	...
	if ((Verification == 'SelfAuthorize')) {
		...
		PaymentVerification()
		GlobalVariable.CurrencyTypeDestino = "NA"
		GlobalVariable.destino_before_db = GlobalVariable.destino_before_app = GlobalVariable.destino_after_db = GlobalVariable.destino_after_app = "Not Applicable for Impuesto case"
...
def Servicios_Monto() {
	...
	if(Mobile.waitForElementPresent(findTestObject('ImpuestosYServicios/MontoTotal'), 10, FailureHandling.OPTIONAL)) {
		Mobile.tap(findTestObject('ImpuestosYServicios/MontoTotal'), 5, FailureHandling.CONTINUE_ON_FAILURE)
