...
'TRANSFERENCIA INTERBANCARIO BENEFICIARIO LBTR : '
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		if (CtaDestino == '') {
			...
			String Attribute = Mobile.getAttribute(findTestObject('Interbancario_Beneficiary/Beneficiarios'), 'enabled', 2,
				FailureHandling.CONTINUE_ON_FAILURE)
	
			if ((Attribute == 'True') || (Attribute == 'true')) {
				Mobile.tap(findTestObject('Interbancario_Beneficiary/Beneficiarios'), 3, FailureHandling.OPTIONAL)
		...
		WebUI.callTestCase(findTestCase('CommonTestCases/TC_PaymentMethod'), [('Method') : Method], FailureHandling.STOP_ON_FAILURE)
