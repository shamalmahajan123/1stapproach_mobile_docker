...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		if(GlobalVariable.DotButton == 'false' || GlobalVariable.DotButton == 'False') {
			//CustomKeywords.'helper.customfunction.Screenshot'()
			KeywordUtil.markPassed('Restric Decimal Scenario Passed - ' + Description)
			GlobalVariable.PaymentStatus = 'Failed'
		} else if (GlobalVariable.DotButton == '' || GlobalVariable.DotButton == null) {
			println('Dot Button Not Found For Decimal Entry')
			GlobalVariable.PaymentStatus = 'Failed'
		} else {
			if(Mobile.waitForElementPresent(findTestObject('PaymentMessages/Por favor complete el campo transaccin no vlida'), 1, FailureHandling.OPTIONAL)){
				KeywordUtil.markPassed('Error Occured - Por favor complete el campo transaccin no vlida..!!')
			} else {
				CustomKeywords.'helper.customfunction.Screenshot'()
			}
		}
		...
		if(Mobile.verifyElementExist(findTestObject('TransferenciaPropia/Digita un concepto'), 2, FailureHandling.OPTIONAL)) {
			...
			KeywordUtil.markPassed('Concepto Is Not Necessary For Test Case...!!!')
...
//		if((Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/android.widget.TextView - Destino present'), 2, FailureHandling.OPTIONAL)
//			|| Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/android.widget.TextView - Origen prsent'), 2, FailureHandling.OPTIONAL))
//			&& (Mobile.verifyElementNotVisible(findTestObject('TransferenciaPropia/android.widget.TextView - e not present'), 1, FailureHandling.OPTIONAL)||
//			Mobile.verifyElementNotVisible(findTestObject('TransferenciaPropia/android.widget.TextView - e origin not presents'), 1, FailureHandling.OPTIONAL))) {
//			CustomKeywords.'helper.customfunction.Screenshot'()
//		}
		
		'CHECK FOR AUTHORIZATION CODE : '
		if(Mobile.waitForElementPresent(findTestObject('ErrorMessages/Para continuar la transaccin se requiere cdigo de autorizacin'), 5, FailureHandling.OPTIONAL)) {
			KeywordUtil.markPassed('Autorization Code Required For Case - ' + Description)
			CustomKeywords.'helper.customfunction.Screenshot'()
		} else {
			'VERIFICATION CODIGO : '
