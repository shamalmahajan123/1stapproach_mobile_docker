...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		WebUI.callTestCase(findTestCase('CommonTestCases/TC_Monto - Tarjeta'), [('Monto') : Monto, ('Decimal') : Decimal, ('MontoCurrency') : MontoCurrency, ('MontoType') : MontoType], FailureHandling.STOP_ON_FAILURE)
		...
		if(GlobalVariable.DotButton == 'false') {
			...
			//Check for Complete action objects
			if(Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo balance a la fecha'), 1, FailureHandling.OPTIONAL) || Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo balance al corte'), 1, FailureHandling.OPTIONAL) || Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo pago mnimo'), 1, FailureHandling.OPTIONAL)) {
				CustomKeywords.'helper.customfunction.Screenshot'()
				KeywordUtil.markFailedAndStop("***Por favor complete el campo message is appearing***")
			}
