...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Type == 'NA') {
		...
		Mobile.delay(2)
		Mobile.tap(findTestObject('ImpuestosYServicios/Recargas - Title'), 3, FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		Mobile.tap(findTestObject('ImpuestosYServicios/Proveedor - Title'), 5, FailureHandling.OPTIONAL)
		if(Description.contains('PagoUnico')) {
			CustomKeywords.'helper.customfunction.DestinoPagoUnicoRecarga'(ImpuestosYServicios, CtaDestino)
		} else {
			CustomKeywords.'helper.customfunction.DestinoAlticeRecarga'(CtaDestino)
		}
		...
		if(Mobile.waitForElementPresent(findTestObject('ImpuestosYServicios/Monto - Title'), 5, FailureHandling.CONTINUE_ON_FAILURE)) {
