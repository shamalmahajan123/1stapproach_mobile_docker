...
'PAGO TARJETA CREDITO PROPIA DOP/USD : '
WebUI.comment((CaseNo + ' - ') + Description)

'PROFILE VERIFICATION FOR DATABASE CONNECTION : '
WebUI.callTestCase(findTestCase('CommonTestCases/TC_ProfileVerification'), [('Verification') : Verification, ('Description') : Description,('CuentaOrigen') : CuentaOrigen,('CuentaDestino') : CuentaDestino, ('MontoCurrency') : MontoCurrency], FailureHandling.STOP_ON_FAILURE)
...
if ((Verification == 'Import') && Role == 'Pasaporte') {
   ...
   WebUI.callTestCase(findTestCase('TC-TokenDigitalMigration/TC_DigitalTokenMigration'), [('Verification') : Verification, ('Cedula') : Cedula,('Password') : Password,('Role') : Role,('Email') : Email], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if (Mobile.waitForElementPresent(findTestObject('TransferenciaPropia/Origen'), 30, FailureHandling.OPTIONAL)) {
		...
		WebUI.callTestCase(findTestCase('CommonTestCases/TC_Monto - Tarjeta'), [('Monto') : Monto, ('Decimal') : Decimal, ('MontoCurrency') : MontoCurrency, ('MontoType') : MontoType, ('Description') : Description], FailureHandling.CONTINUE_ON_FAILURE)
		...
		if(GlobalVariable.DotButton == 'false') {
			...
			if(checkMonto.matches(".*[a-zA-Z]+.*")) {
				...
				if(MontoCurrency == "DOP") {
					...
					Monto = Double.toString((Double.parseDouble(GlobalVariable.MontoDecimal)*GlobalVariable.USD_to_DOP_rate));
				}
			} else {
			...
			//Convert tax value for USD case & Change CurrencyTypeOrigen value
			if(MontoCurrency == "USD") {
				GlobalVariable.montoReplace = "USD"
				double tax = GlobalVariable.GetImpuesta
				tax/=GlobalVariable.USD_to_DOP_rate
				String taxUSD = String.format("%.2f", tax)
				GlobalVariable.GetImpuesta = Double.parseDouble(taxUSD)
				//KeywordUtil.logInfo(GlobalVariable.GetImpuesta)
			} else {
				GlobalVariable.montoReplace = "DOP"
			}
...
def PaymentVerification() {
	if (Verification == 'SelfAuthorize') {
		...
		'VERIFY THE BALANCES : '
		WebUI.callTestCase(findTestCase('CommonTestCases/TC_VerifyResultsModified'), [('Verification') : Verification,('Description') : Description,('CuentaOrigen') : CuentaOrigen, ('CuentaDestino') : CuentaDestino,('CtaOrigen') : CtaOrigen, ('CtaDestino') : CtaDestino,('Monto') : Monto, ('Impuesto') : Impuesto, ('MontoCurrency') : MontoCurrency], FailureHandling.OPTIONAL)
